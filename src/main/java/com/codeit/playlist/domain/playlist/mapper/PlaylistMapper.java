...
public interface PlaylistMapper {
    ...
    PlaylistDto toDto(Playlist playlist, @Context Map<UUID, List<String>> tagMap,
                      @Context S3Properties s3Properties);

    //태그가 필요없는 곳에 쓰일 편의 메서드
    default PlaylistDto toDto(Playlist playlist, @Context S3Properties s3Properties) {
        return toDto(playlist, java.util.Collections.emptyMap(), s3Properties);
    }
    ...
    @Mapping(target = "thumbnailUrl", source = "content.thumbnailUrl", qualifiedByName = "toS3Url")
    @Mapping(target = "averageRating", source = "content.averageRating")
    @Mapping(target = "reviewCount", source = "content.reviewCount")
    @Mapping(target = "tags",
            expression = "java(tagMap.getOrDefault(playlistContent.getContent().getId(), java.util.List.of()))")
